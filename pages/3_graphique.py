...
nb1 = int(nb1.number_input("x min"))
nb2 = int(nb2.number_input("x max"))
n = 0

txt = st.text_area(label="Fonction", value=st.session_state["ecran"], disabled=True, placeholder="0")
...
def add_multiplication():
    st.session_state["ecran"] += ("*")
def addcos():
    st.session_state["ecran"]+=("cos(")
def addN():
    st.session_state["ecran"] += ("n")
def tracer():

    def Un():
        return [eval(txt) for n in range(nb1,nb2+1)]

    liste_n = [n for n in range(nb1,nb2+1)]

    valeur = Un()


    dic = dict(zip(liste_n,valeur))
...
def addparentheseD(): 
    st.session_state["ecran"] += (")")
def addtrig():
    st.session_state["ecran"] += (st.session_state.fonction_trig)


buttonN,button0 ,button1, button2, button3 = st.columns([1,1,1,1,1])

buttonN.button("n", on_click=addN)
...
cosinus, addition, button4, button5, button6 = st.columns([1,1,1,1,1])

if not 'fonction_trig' in st.session_state:
    st.session_state['fonction_trig'] = None

st.session_state['fonction_trig'] = cosinus.selectbox("trig", placeholder="fonction trigo",options=["cos", "sin", "tan", "acos", "acsin", "actan"],index=None)
st.write(st.session_state.fonction_trig)

if st.session_state.fonction_trig is not None:
    st.session_state["ecran"] += (st.session_state.fonction_trig)
#     st.write(st.session_state.fonction_trig)
#     addtrig()
...
soustraction, button7, button8, button9, parentheseD = st.columns([1,1,1,1,1])
...
parentheseD.button(")", on_click=addparentheseD)
    
multiplication, division, afficher, parentheseG = st.columns([1,1,2,1])

multiplication.button("x", on_click=add_multiplication)
division.button("/", on_click=add_division)
if afficher.button("Tracer", use_container_width=True):
    tracer()

parentheseG.button("(", on_click=addparentheseG)
